...
//                                    startActivity(intent);
                                    ...
                                    BottomSheetDialog dialog = new BottomSheetDialog(OtpVerifyActivity.this, R.style.AppBottomSheetDialogTheme);
                                    View dialogView = LayoutInflater.from(getApplicationContext()).inflate(R.layout.activity_change_password,null);
                                    dialog.setContentView(dialogView);

                                    EditText changePass = dialogView.findViewById(R.id.changePass);
                                    EditText confirmChange = dialogView.findViewById(R.id.confirmChange);
                                    ...
                                    btnChange.setOnClickListener(new View.OnClickListener() {
                                        ...
                                        public void onClick(View view) {
                                            String newChange = changePass.getText().toString();
                                            String newConfirm = confirmChange.getText().toString();
                                            
                                            if (newConfirm.isEmpty() || newChange.isEmpty()){
                                                Toast.makeText(OtpVerifyActivity.this, "Fill all form", Toast.LENGTH_SHORT).show();
                                            }else {
                                                if (newConfirm.length() != newChange.length()){
                                                    Toast.makeText(OtpVerifyActivity.this, "New password not match", Toast.LENGTH_SHORT).show();
                                                }else {
                                                    if (newChange.length() <=7){
                                                        Toast.makeText(OtpVerifyActivity.this, "New password is too short\nLength must be 8 above", Toast.LENGTH_SHORT).show();
                                                    }
                                                    Intent intent = new Intent(OtpVerifyActivity.this, MainActivity.class);
                                                    intent.putExtra("changePass", changePass.getText().toString());
                                                    Toast.makeText(OtpVerifyActivity.this, "Change password Succeeded\nWelcome...", Toast.LENGTH_SHORT).show();
                                                    startActivity(intent);
                                                }
                                            }
                                        }
